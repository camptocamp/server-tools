...
class JsonifyStoredMixin(models.AbstractModel):
    ...
    _jsonify_lang_field__cache = None

    @property
    def _jsonify_lang_field(self):
        """Field providing specific language per record."""
        if not self._jsonify_lang_field__cache:
            self._jsonify_lang_field__cache = DotDict(
                {
                    "name": "lang_id",
                    "exists": "lang_id" in self._fields,
                }
            )
        return self._jsonify_lang_field__cache

    @staticmethod
    def _jsonify_data_grouper_and_sorter(rec):
        lang = None
        if rec._jsonify_lang_field.exists:
            # Support translated records.
            lang = rec[rec._jsonify_lang_field]
    ...
    def cron_update_json_data_for(
        ...
        domain = domain or []
        if model._jsonify_lang_field.exists and not lang_domains:
    ...
    def _json_data_compute_get_lang_domains(self):
        ...
        # '__domain': [('lang_id', '=', 30)]}]
        by_lang = self.read_group([], ["id"], [self._jsonify_lang_field])
