...
class TestRegistryCacheCustom(TransactionCase):
    ...
    def tearDown(self):
        ...
        registry._REGISTRY_CACHES = self.__REGISTRY_CACHES_BACKUP
        registry._CACHES_BY_KEY = self.__CACHES_BY_KEY_BACKUP
        super().tearDown()
    ...
    def test_03_add_custom_cache_existing(self):
        """Checks a ``ValueError`` is raised when trying to add an existing cache"""
        add_custom_cache("test_cache", 10)
        with self.assertRaisesRegex(
            CacheAlreadyExistsError, r"^Cache 'test_cache' already exists$"
        ):
            add_custom_cache("test_cache", 10)

    @mute_logger("odoo.addons.base_registry_cache_custom.registry")
    def test_04_add_custom_cache_dotted_name_invalid(self):
        """Checks a ``ValueError`` is raised when trying to use a dotted name"""
        with self.assertRaisesRegex(
    ...
    def test_05_add_custom_cache_dependent(self):
        ...
        # Put some dumb data in these caches, then invalidate the parent cache and check
        # that the dependent cache is empty
        db_registry = self.env.registry
        db_registry._Registry__caches["test_cache_1"]["key"] = 1
        db_registry._Registry__caches["test_cache_2"]["key"] = 1
        db_registry.clear_cache("test_cache_1")
        self.assertEqual(len(db_registry._Registry__caches["test_cache_2"]), 0)
