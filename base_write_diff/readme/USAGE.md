...
By using context key ``"write_use_diff_values"``: if ``True``-ish, the
 ``BaseModel.write()`` override will take care of filtering out the field values that
 are the same as the record's current ones. You can use this to override a model's
 ``write()`` method to prevent unnecessary changes, or use it to make sure you don't
 update a field unless its current value is different from the one you want to set.
...
- you'll be able to filter out specific fields, instead of
  filtering out all the fields whose values won't be changed after the update;
